...
public class CompanyServiceImpl implements CompanyService {
	...
	public CompanyDto login(String email, String password) {
		return companyRepository.findByEmail(email).filter(company -> company.getPassword().equals(password))
				.map(this::mapToDto).orElseThrow(() -> new RuntimeException("Invalid email or password"));
	}
